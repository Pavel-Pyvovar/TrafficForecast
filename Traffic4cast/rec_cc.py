...
parser.add_argument('--city', type=str, default="london",
                    help="london, melbourne, madrid")  # city
parser.add_argument('--split', type=float, default=0.8,
                    help="train:test=0.8:0.2")
...
parser.add_argument('--hidden_channels', type=int,
                    default=32, help="hidden_channels")
parser.add_argument('--num_layers', type=int, default=3,
                    help="num_layers for predict model")

parser.add_argument('--batch_size', type=int, default=2, help="batch_size")
parser.add_argument('--epochs', type=int, default=20, help="epochs")

# if enriched = True make sure that edge_attributes specified correctly
parser.add_argument('--enriched', type=bool, default=True,
                    help="whether to use enriched road edges or not")
parser.add_argument('--num_edge_classes', type=int,
                    default=3, help="num_edge_classes")
...
opt = vars(args)
opt[
    "save_path"] = f"{str(BASEDIR)}/results/{opt['model_state']}/{opt['city']}/class_L{str(opt['num_layers'])}_H{str(opt['hidden_channels'])}_F{str(opt['fill'])}_N{str(opt['normalize'])}_B{str(opt['batch_size'])}_e{str(opt['epochs'])}/"
opt[
...
class RecLinear(nn.Module):
    def __init__(self, num_edges, num_nodes, num_attrs, in_channels, hidden_channels, out_channels, num_layers):
        ...
        for i in range(3):
            self.conv1.append(
                GCNConv(hidden_channels, hidden_channels, edge_dim=hidden_channels))
    ...
    def forward(self, index, edge_index, x, attr, cur_t, cur_w):
        mask_idx = (torch.sum(x, dim=1, keepdim=True) !=
                    nan_to_num_map[opt['city']] * 4).type(torch.float)
        ...
        xf = torch.cat([embed, self.attr_lin1(attr), x,
                       x1, time_embed, week_embed], dim=1)
...
# data loader
dataset = T4c22GeometricDataset(root=BASEDIR,
                                city=opt['city'],
                                edge_attributes=['parsed_maxspeed', 'speed_kph', 'length_meters', 'counter_distance', 'num_lanes', 'mean_pop',
                                                 'catering', 'leisure', 'shopping', 'tourism', 'traffic', 'transport',
                                                 'pois_total', 'greeneries', 'water_bodies', 'built_up', 'other', 'importance', 'highway',
                                                 'oneway',],
                                enriched=opt['enriched'],
                                split="train",
                                fill=opt['fill'],
                                normalize=opt['normalize'],
                                cachedir=Path(f"{BASEDIR}/cache"),
                                idx=opt['cluster'])
...
if __name__ == "__main__":
    ...
    np.save(
        f"{str(BASEDIR)}/temp/{opt['city']}_edge_attr.npy", edge_attr.cpu().numpy())
    ...
    if opt['model_state'] == "train":
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
            ...
            for epoch in tqdm.tqdm(range(1, opt['epochs']+1), "epochs", total=opt['epochs']):
                ...
                for data in pbar:
                    ...
                    for i in range(data.y.shape[0] // lens1):
                        ...
                        train_index = torch.nonzero(
                            torch.sum(x, dim=1) != nan_to_num_map[opt['city']] * 4).squeeze()
                    ...
                    pbar.set_postfix(rec_loss=rec_loss.cpu(
                    ).item(), acc_loss=acc_loss.cpu().item())
                ...
                if (np.mean(losses) < min_loss):
                    ...
                    torch.save(model.state_dict(),
                               f"{opt['save_path']}model_best_%d.pt" % fold)
                torch.save(model.state_dict(),
                           f"{opt['save_path']}model_{fold}_{epoch:03d}.pt")
    else:
            ...
            if (opt['city'] == 'melbourne'):
                ...
                for fold in range(5):
                    model.load_state_dict(torch.load(
                        f"{opt['save_path']}model_best_{fold}.pt"))
                    model.eval()
                    for ii in range(5):
                        y_hat, _ = model(index, edge_index,
                                         data.x, edge_attr, cur_t, cur_week)
                        y_hat = y_hat.detach()
                        y_pred += y_hat
                y_pred /= 25.
            else:
                ...
                for fold in range(5):
                    for mm in range(16, 21):
                        model.load_state_dict(torch.load(
                            f"{opt['save_path']}model_{fold}_{mm:03d}.pt"))
                        model.eval()
                        for ii in range(5):
                            y_hat, _ = model(
                                index, edge_index, data.x, edge_attr, cur_t, cur_week)
                            y_hat = y_hat.detach()
                            y_pred += y_hat
                y_pred /= 125.

            df = test_dataset.torch_road_graph_mapping._torch_to_df_cc(
                data=y_pred, day="test", t=idx)
            dfs.append(df)
        df = pd.concat(dfs)
        df["test_idx"] = df["t"]
        del df["day"]
        ...
        (BASEDIR / "submissions" / opt['submission_name'] /
         opt['city'] / "labels").mkdir(exist_ok=True, parents=True)
